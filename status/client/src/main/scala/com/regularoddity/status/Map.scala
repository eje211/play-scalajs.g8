...
object Map {
  ...
  def addPin(employee: Employee): Marker = {
    ...
    marker.asInstanceOf[MarkerAddendum].bindPopup(
      s"""<div id="_${employee.id.get}"><pin-label eid="${employee.id.get}"><pin-label><div>"""
        .asInstanceOf[HTMLElement], js.Dictionary("minWidth" -> 200.asInstanceOf[js.Any]))
    marker.on("moveend", e => {
